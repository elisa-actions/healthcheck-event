...
const sendEvent = (healthcheckUrl, canary, token, resourceId, event) => __awaiter(void 0, void 0, void 0, function* () {
    const maxRetries = 3;
    let retryCount = 0;
    const http = new http_client_1.HttpClient('healthcheck-event-action');
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const idToken = yield core.getIDToken(healthcheckUrl);
            ...
            if (!['deployment', 'incident'].includes(type)) {
                core.setFailed(`Invalid event type: ${type}. Must be one of: deployment, incident`);
                return;
            }
            const event = {
                message,
            ...
            const res = yield sendEvent(healthcheckUrl, canary, idToken, resourceId, event);
            ...
            if (res.message.statusCode && res.message.statusCode > 299) {
                const healthcheckErrorResponse = JSON.parse(responseBody);
                core.setFailed(`Failed to send event: ${healthcheckErrorResponse.message}`);
            }
        }
        catch (error) {
